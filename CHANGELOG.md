...
- **`api_key_env` secret exfiltration guard**: `llm_call` and `memory` handlers block workflow-controlled `api_key_env` from reading the engine's own secrets (`ORCH8_*` namespace) and well-known infrastructure env vars (`DATABASE_URL`, `AWS_SECRET_ACCESS_KEY`, `GITHUB_TOKEN`, etc.). Only legitimate provider keys (e.g. `OPENAI_API_KEY`) are allowed.
