...
impl EncryptingStorage {
    ...
    fn encrypt_credential(
        ...
        // Encrypt value: wrap the raw secret string as a JSON string value, encrypt,
        // then extract the resulting string to store.
    ...
    fn decrypt_credential(
        ...
        // Decrypt value if it carries the encryption prefix.
        ...
        // Decrypt refresh_token if present and encrypted.
...
impl StorageBackend for EncryptingStorage {
    ...
    async fn create_instances_batch(
        ...
        // Fast path: if every instance's `context.data` is already encrypted,
        // forward the borrowed slice directly and skip the per-item clone the
        // `Cow::Owned` path would otherwise force into an owned `Vec`.
        if instances
            .iter()
    ...
    async fn merge_context_data(
        ...
        // encrypted string (`"enc:v1:..."`) — NOT a JSONB object. The
        // underlying `jsonb_set(context, ARRAY['data', $key], $value)`
        // implementation guards against non-object `data` by replacing it
        // with `{}` before the set; on encrypted rows that guard silently
        // wipes the entire existing (encrypted) context. That is
        // irreversible data loss — the ciphertext is gone and no decrypt
        // can recover it.
        //
        // The only safe path is read-merge-reencrypt-write under a single
        // get/update pair. We lose the single-round-trip benefit of a
        // server-side `jsonb_set`, but callers get correct semantics.
        //
        // Note: this is still racy against concurrent writers to the same
        // instance's context. The previous non-encrypted path was also racy
        // against concurrent `update_instance_context`, so encryption mode
        // does not strictly widen that window — but concurrent
        // `merge_context_data` calls on disjoint keys (which the unencrypted
        // path handles safely via `jsonb_set` on independent paths) now race
    ...
    // === Credentials (encrypt value + refresh_token at rest) ===
