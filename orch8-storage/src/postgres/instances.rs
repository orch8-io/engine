...
/// Atomically increment `context.runtime.total_steps_executed` via `jsonb_set`
...
pub(super) async fn increment_total_steps(
    ...
    let new_total: Option<i64> = sqlx::query_scalar(
        "UPDATE task_instances \
         SET context = jsonb_set(context, ARRAY['runtime', 'total_steps_executed'], \
             to_jsonb(COALESCE((context #>> '{runtime,total_steps_executed}')::int, 0) + 1)), \
             updated_at = NOW() \
         WHERE id = $1 \
         RETURNING (context #>> '{runtime,total_steps_executed}')::bigint",
    )
    .bind(id.into_uuid())
    .fetch_optional(&store.pool)
    .await?;
...
pub(super) async fn list_artifact_gc_candidates(
    ...
    // new terminal state is added — the SQLite twin carries the same list.
    let rows = sqlx::query(
        "SELECT id FROM task_instances \
         WHERE state::text IN ('completed','failed','cancelled') \
           AND updated_at < $1 \
           AND (metadata->>'_artifacts_gced') IS NULL \
...
pub(super) async fn mark_artifacts_gced(
    ...
    sqlx::query(
        "UPDATE task_instances \
         SET metadata = jsonb_set(COALESCE(metadata, '{}'::jsonb), '{_artifacts_gced}', 'true'::jsonb, true) \
         WHERE id = $1",
    )
    .bind(instance_id.into_uuid())
    .execute(&store.pool)
