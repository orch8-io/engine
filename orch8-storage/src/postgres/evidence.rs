...
#[async_trait]
impl crate::InvariantStore for PostgresStorage {
    ...
    async fn list_workflow_invariants(
        ...
        let rows = sqlx::query(
            "SELECT record FROM workflow_invariants
             WHERE tenant_id = $1 AND sequence_id = $2 AND enabled
               AND (sequence_version IS NULL OR sequence_version = $3)
             ORDER BY created_at, id LIMIT $4",
        )
        .bind(tenant_id.as_str())
        .bind(sequence_id.into_uuid())
    ...
    async fn append_invariant_result(
        ...
        let inserted = sqlx::query(
            "INSERT INTO invariant_results
             (id, invariant_id, tenant_id, continuity_id, epoch, status,
              dedupe_key, record, evaluated_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
             ON CONFLICT (tenant_id, dedupe_key) DO NOTHING",
        )
        .bind(result.id.into_uuid())
        .bind(result.invariant_id.into_uuid())
        .bind(tenant_id.as_str())
        .bind(result.continuity_id.into_uuid())
        .bind(
            i64::try_from(result.epoch.get())
                .map_err(|_| StorageError::Query("invariant result epoch exceeds BIGINT".into()))?,
        )
...
#[async_trait]
impl crate::EvaluationStore for PostgresStorage {
    async fn append_evaluation_score(&self, score: &EvaluationScore) -> Result<bool, StorageError> {
        ...
        .bind(score.id.into_uuid())
        .bind(score.tenant_id.as_str())
        .bind(score.continuity_id.into_uuid())
    ...
    async fn list_evaluation_scores(
        ...
        let rows = sqlx::query(
            "SELECT record FROM evaluation_scores
             WHERE tenant_id = $1 AND continuity_id = $2
             ORDER BY created_at DESC, id DESC LIMIT $3",
        )
        .bind(tenant_id.as_str())
...
#[async_trait]
impl crate::AttentionStore for PostgresStorage {
    async fn create_attention_task(&self, task: &AttentionTask) -> Result<(), StorageError> {
        ...
        .bind(task.continuity_id.into_uuid())
    ...
    async fn get_attention_task(
        ...
        let row = sqlx::query("SELECT record FROM attention_tasks WHERE tenant_id=$1 AND id=$2")
            .bind(tenant_id.as_str())
            .bind(id.into_uuid())
    ...
    async fn claim_attention_task(
        ...
        let updated = sqlx::query(
            "UPDATE attention_tasks SET state=$1, assignee=$2, lease_expires_at=$3, record=$4
             WHERE tenant_id=$5 AND id=$6 AND state=$7 AND deadline>$8",
        )
    ...
    async fn reserve_budget(
        ...
        let locked = sqlx::query(
            "SELECT continuity_id FROM continuity_executions
             WHERE tenant_id=$1 AND continuity_id=$2 AND epoch=$3 FOR UPDATE",
        )
        .bind(reservation.tenant_id.as_str())
        .bind(reservation.continuity_id.into_uuid())
        .bind(
            i64::try_from(reservation.epoch.get())
                .map_err(|_| StorageError::Query("budget epoch exceeds BIGINT".into()))?,
        )
