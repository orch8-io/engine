...
## 2025-10-31 - [Avoid Vector Allocations in Execution Tree Traversals]
**Learning:** In `orch8-engine/src/evaluator.rs`, the helper function `children_of` iterates over `tree`, filters for a `parent_id`, and `.collect()`s references into a new `Vec`. In functions like `is_inside_decided_race` and `has_racing_composite_sibling`, this was used within an `.into_iter().any(...)` loop. This introduces unnecessary intermediate `Vec` memory allocations when traversing execution trees in hot loops. Using `.iter().any(|c| ...)` directly on `tree` avoids allocating the intermediate `Vec`, improving performance by short-circuiting and operating fully zero-allocation.
**Action:** To avoid intermediate `Vec` memory allocations when traversing execution trees in hot loops (like in `orch8-engine/src/evaluator.rs`), prefer using direct iteration and short-circuiting methods (e.g., `tree.iter().any(|c| ...)`) over helper functions that collect and return vectors (e.g., `children_of(...).into_iter().any(...)`).
