...
## 2025-05-18 - [Zero-Allocation Enum Display Bindings]
**Learning:** `sqlx` parameters for enum strings like `InstanceState` and `WorkerTaskState` were bound using `.to_string()` when saving lists, which internally delegates to the `Display` trait and heap-allocates a new string literal each time. This creates unnecessary allocations inside high-throughput iteration paths like `push_values` and batch creations.
**Action:** Always implement and prefer an `as_str() -> &'static str` method on variants mapping exactly to static strings when storing these string states. This enables zero-allocation string binding via `push_bind()` bypassing the heap allocation of `.to_string()`. Use `as_str()` locally inside the `Display` implementation.
