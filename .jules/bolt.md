...
## 2025-10-31 - [Zero-Allocation Tree Indexing Iteration]
**Learning:** In execution hot loops within `orch8-engine` (specifically `is_inside_decided_race` and `has_racing_composite_sibling`), checking child state using `children_of(...).into_iter().any(...)` causes a temporary `Vec` of references to be allocated on each iteration, generating unnecessary memory allocation overhead. By replacing this with a direct loop on the tree (`tree.iter().any(...)`), the code can take full advantage of `.any()`’s short-circuiting and evaluate lazily without allocating any intermediate collections.
**Action:** Always prefer direct iteration combined with `.any()` or `.all()` over helper functions that `.collect()` intermediate results into vectors on performance-critical execution hot paths to eliminate memory allocation overhead.
