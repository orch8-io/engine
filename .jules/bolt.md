...
## 2026-08-23 - [Zero-Allocation ExecutionNode State Comparison]
**Learning:** In the composite reevaluation hot path within `orch8-engine/src/evaluator.rs`, eagerly allocating multiple `Vec<(ExecutionNodeId, NodeState)>` snapshots of the execution tree solely to verify if state changes occurred creates unnecessary memory allocation overhead.
**Action:** When comparing snapshots of structurally stable slices (like `[ExecutionNode]`) to detect mutations, prefer a zero-allocation helper function that compares lengths and iterates with `.zip().any()` to short-circuit upon the first mismatch.
