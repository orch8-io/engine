...
## 2026-09-01 - [Zero-Allocation Tree State Snapshotting]
**Learning:** In the `orch8-engine/src/evaluator.rs` fast path `evaluate` loop, snapshotting `ExecutionNode` states using `ctx.tree.iter().map(|n| (n.id, n.state)).collect()` into a `Vec` for pre, mid, and post state comparisons causes significant memory allocation overhead. A simple length check followed by `tree.iter().zip(new_tree.iter()).any(...)` can perform this exact same validation inline, returning a boolean for early restarts, short-circuiting as soon as a difference is found, and eliminating all intermediate heap allocations for tree state tracking.
**Action:** When comparing the structure or state of slices (like execution trees before and after an operation), never collect elements into an intermediate `Vec`. Use `len() != len()` combined with `.iter().zip().any(...)` for zero-allocation structural comparisons in hot paths.
