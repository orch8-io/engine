...
impl ManifestGenerator {
    ...
    /// Raise the version floor to `version` (e.g. the `manifest_version` of
    /// the manifest currently published, as persisted or read back by the
    /// caller), so the next manifest is strictly newer even when it is
    /// generated by a different process or after a wall-clock regression.
    pub fn observe_published_version(&self, version: i64) {
        self.last_version
            .fetch_max(version, std::sync::atomic::Ordering::Relaxed);
    }

    /// Highest manifest version handed out or observed so far (0 if none).
    pub fn last_version(&self) -> i64 {
        self.last_version.load(std::sync::atomic::Ordering::Relaxed)
    }
...
mod tests {
    ...
    #[test]
    fn observed_version_is_a_floor() {
        let r#gen = ManifestGenerator::new(test_key(), "key1".to_string());
        let far_future = Utc::now().timestamp_millis() + 10_000_000;
        r#gen.observe_published_version(far_future);
        let m = r#gen.generate(vec![], vec![], vec![]).unwrap();
        assert_eq!(m.body.manifest_version, far_future + 1);
        // A lower observation never moves the floor backwards.
        r#gen.observe_published_version(1);
        let m2 = r#gen.generate(vec![], vec![], vec![]).unwrap();
        assert_eq!(m2.body.manifest_version, far_future + 2);
    }
