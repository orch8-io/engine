...
export function Button({
  ...
  return (
    <button
      {...rest}
      className={`inline-flex items-center gap-1.5 rounded-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${sz} ${VARIANT[variant]} ${className}`}
    >
      {children}
    </button>
  );
