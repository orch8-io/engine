...
//! step params via `credentials://<id>`. Responses redact `value` and
//! `refresh_token` automatically via `SecretString`'s `Serialize` impl —
//! operators can only write secrets, never read them back through this API.
