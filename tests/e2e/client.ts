...
export class Orch8Client {
  ...
  async listCircuitBreakers(): Promise<ApiResponse[]> {
    return this.#get<ApiResponse[]>("/circuit-breakers");
  }

  async getCircuitBreaker(handler: string): Promise<ApiResponse> {
    return this.#get(`/circuit-breakers/${handler}`);
  }

  async resetCircuitBreaker(handler: string): Promise<ApiResponse> {
