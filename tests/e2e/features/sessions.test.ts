...
describe("Sessions", () => {
  ...
  it("creates, reads by id and by key, and returns expected fields", async () => {
    ...
    const byId = await client.getSession(created.id as string);
  ...
  it("patches data and state", async () => {
    ...
    await client.updateSessionData(created.id as string, { v: 2, new_field: "x" });
    await client.updateSessionState(created.id as string, "completed");
  ...
  it("GET /sessions/{id}/instances returns an array", async () => {
    ...
    const instances = await client.listSessionInstances(created.id as string);
