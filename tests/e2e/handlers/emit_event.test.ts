...
import type { BlockOutput } from "../client.ts";
 ...
 */
async function createTrigger(body: Record<string, unknown>): Promise<unknown> {
 ...
 */
function findOutput(outputs: BlockOutput[], blockId: string): any {
...
describe("emit_event / query_instance / dedupe E2E", () => {
  ...
  it("producer emits event → child reaches terminal state with inherited context", async () => {
    ...
    const finished = await client.waitForState(producerId, "completed", { timeoutMs: 20_000 });
    ...
    assert.deepEqual((child.context as any).data, { hello: "from-parent" });

    // Child's metadata records the parent_instance_id set by emit_event.
    assert.equal((child as any).metadata._trigger, slug);
    assert.equal((child as any).metadata._trigger_event.source, "emit_event");
    assert.equal((child as any).metadata._trigger_event.parent_instance_id, producerId);
  });
  ...
  it("two emit_event calls with same dedupe_key → only one child instance", async () => {
    ...
    await client.waitForState(producerId, "completed", { timeoutMs: 20_000 });
