...
try {
  ...
  // Heartbeat: ping /health/live every second and log the instant the
  // server stops responding. Held in the module-level `heartbeatRef` so
  // `shutdown()` can clear it. This pinpoints the crash window against
  // the spec-reporter output so we can correlate the death with the
  // currently-running suite.
  let lastHealthy = true;
  heartbeatRef = setInterval(async () => {
    try {
      const res = await fetch(`http://localhost:${PORT}/health/live`, {
        // Short timeout so a hung server reports "unreachable" rather
        // than queuing health pings behind the dead socket.
        signal: AbortSignal.timeout(500),
      });
      if (!res.ok && lastHealthy) {
        lastHealthy = false;
        console.log(
          c.red(
            `  [heartbeat] /health/live -> ${res.status} at ${new Date().toISOString()}`,
          ),
        );
      }
    } catch (e) {
      if (lastHealthy) {
        ...
        const exitSig = handle?.child?.signalCode;
        console.log(
          c.red(
            `  [heartbeat] server unreachable at ${new Date().toISOString()} — ${msg} (child.exitCode=${exitCode} signal=${exitSig})`,
          ),
        );
      }
    }
  }, 1_000);
  // Unref so the interval doesn't keep the event loop alive during
  // shutdown — shutdown() already clears it explicitly, this is belt-
  // and-braces.
  heartbeatRef.unref();
