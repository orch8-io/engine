...
describe("Resource Pools + Checkpoints", () => {
  ...
  it("pool CRUD: create, list, get, update resource, delete", async () => {
    ...
    // Get by id.
    const got = await client.getPool(pool.id);
    ...
    const updated = await client.updatePoolResource(pool.id, resource.id, {
      enabled: false,
      daily_cap: 500,
    });
    ...
    const after = await client.listPoolResources(pool.id);
    assert.ok(!after.some((r) => r.id === resource.id));

    // Delete the pool.
    await client.deletePool(pool.id);
    try {
      await client.getPool(pool.id);
  ...
  it("pool list is tenant-scoped", async () => {
    ...
    // Cleanup.
    await client.deletePool(poolA.id);
    await client.deletePool(poolB.id);
  });
  ...
  it("save + list + prune checkpoints on a completed instance", async () => {
    ...
    assert.ok(pruneRes.count >= 2, `expected >=2 pruned, got ${pruneRes.count}`);
