 ...
 */
function race(id, branches, semantics) {
  const block = { type: "race", id, branches };
 ...
 */
function tryCatch(id, tryBlock, catchBlock, finallyBlock) {
  const block = { type: "try_catch", id, try_block: tryBlock, catch_block: catchBlock };
 ...
 */
function forEach(id, collection, body, opts = {}) {
...
describe("Composite Blocks", () => {
  ...
  it("race: all branches fail => race fails", async () => {
    ...
    const failed = await client.waitForState(id, "failed", { timeoutMs: 10000 });
    assert.equal(failed.state, "failed");
  });
  ...
  it("forEach: iterates over collection from context", async () => {
    const seq = testSequence("fe-basic", [
      forEach("fe1", "items", [step("body_step", "noop")]),
    ]);
    await client.createSequence(seq);
    ...
    const completed = await client.waitForState(id, "completed", { timeoutMs: 15000 });
    assert.equal(completed.state, "completed");
  });
  ...
  it("forEach: missing collection completes (no-op)", async () => {
    const seq = testSequence("fe-missing", [
      forEach("fe1", "nonexistent", [step("body_step", "noop")]),
    ]);
    await client.createSequence(seq);
