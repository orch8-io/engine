...
describe("Worker retry / backoff semantics", () => {
  ...
  // Plan #85 (engine-accurate): `retry.backoff_multiplier` only affects
  // the *in-process* handler path via `scheduler.rs::handle_retryable_failure`.
  // The worker-task API path (`orch8-api/src/workers.rs::fail_task`)
  // re-schedules with `next_fire_at = now` and does NOT honor
  // `initial_backoff` / `backoff_multiplier`. Asserting wall-clock gaps
  // over the external-worker path would test behavior that is not
  // implemented. The "retries happen up to max_attempts" contract is
