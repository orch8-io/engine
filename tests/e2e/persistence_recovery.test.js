...
describe("Data Persistence and Recovery", () => {
  ...
  it("preserves instance state across server restart", async () => {
    ...
    const seq = testSequence("restart-test", [
      step("s1", "sleep", { duration_ms: 10000 }), // 10 second sleep
    ]);
  ...
  it("preserves completed instance data across restart", async () => {
    ...
    // Wait for completion
    await client.waitForState(id, "completed", { timeoutMs: 10000 });
    ...
    assert.ok(logOutput);
    assert.equal(logOutput.output.message, "test message");
  });
  ...
  it("prunes old checkpoints while keeping specified number", async () => {
    ...
    const keptIndices = prunedCheckpoints.map(cp => cp.checkpoint_data.index);
  ...
  it("handles sequence version updates during execution", async () => {
    ...
    const instance2 = await client.waitForState(id2, "completed", { timeoutMs: 10000 });
  ...
  it("recovers externalized state after restart", async () => {
    ...
    assert.ok(instance.context.data.updated);
    assert.ok(instance.context.data.updateTime);
  ...
  it("recovers worker task state after restart", async () => {
    ...
    if (tasksAfter.length > 0) {
      ...
      try {
        ...
        console.log(`Task completion failed after restart: ${err.message}`);
      }
    }
  });
