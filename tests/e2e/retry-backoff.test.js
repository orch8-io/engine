...
describe("Worker retry / backoff semantics", () => {
  ...
  it("retryable failure re-dispatches a new worker task until max_attempts", async () => {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      await client.waitForState(id, "waiting", { timeoutMs: 10000 });
    ...
    // Eventually Failed.
    const failed = await client.waitForState(id, "failed", { timeoutMs: 15000 });
  ...
  it("failed instance with retry policy ends up in DLQ", async () => {
    ...
    await client.waitForState(id, "failed", { timeoutMs: 15000 });
