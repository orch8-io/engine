...
pub fn mk_step_with_conditional_retry(
    id: &str,
    handler: &str,
    max_attempts: u32,
    retry_if: Option<&str>,
    non_retryable_codes: Option<Vec<&str>>,
) -> BlockDefinition {
    BlockDefinition::Step(Box::new(StepDef {
        id: BlockId::new(id),
        handler: handler.into(),
        ...
        retry: Some(RetryPolicy {
            ...
            max_backoff: Duration::from_millis(10),
            backoff_multiplier: 1.0,
            retry_if: retry_if.map(ToString::to_string),
            non_retryable_codes: non_retryable_codes
                .map(|codes| codes.into_iter().map(ToString::to_string).collect()),
        }),
        timeout: None,
        rate_limit_key: None,
        send_window: None,
