...
async fn race_winner_cancels_other_branches() {
    ...
    let block = BlockDefinition::Race(race_def.clone());
...
async fn race_all_branches_fail_node_fails() {
    ...
    let (storage, instance, tree) = setup(vec![BlockDefinition::Race(race_def.clone())]).await;
    let reg = HandlerRegistry::new();
    let race_node = find_node(&tree, BlockType::Race);
...
async fn parallel_all_branches_succeed_node_completes() {
    ...
    let (storage, instance, tree) = setup(vec![BlockDefinition::Parallel(par.clone())]).await;
    let reg = HandlerRegistry::new();
