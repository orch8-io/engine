...
mod tests {
    ...
    #[tokio::test]
    async fn skips_stale_waiting_instances() {
        // A parked `Waiting` instance (human-review gate / wait-for-event)
        // never heartbeats, so lease age says nothing about its health — the
        // reaper must leave it alone no matter how old `updated_at` is. Its
        // liveness contract is the deadline/timeout sweep
        // (`process_waiting_deadlines`), which this test does not exercise.
        use chrono::Utc;
        use orch8_types::context::ExecutionContext;
        use orch8_types::ids::{InstanceId, Namespace, SequenceId, TenantId};
        use orch8_types::instance::{InstanceState, Priority, TaskInstance};
        ...
        // Parked for 24h, threshold 60s — still must not be recycled.
        let n = recover_stale_instances(&storage, 60).await.unwrap();
        assert_eq!(n, 0);

        let unchanged = storage.get_instance(inst.id).await.unwrap().unwrap();
        assert_eq!(unchanged.state, InstanceState::Waiting);
    }
