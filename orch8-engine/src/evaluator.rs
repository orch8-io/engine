...
pub fn children_of(
    ...
    // ⚡ Bolt: Using a manual loop with a preallocated `Vec` avoids the multiple
    // reallocation overheads of `.filter(...).collect()` on an unknown-sized
    // iterator in execution hot paths.
