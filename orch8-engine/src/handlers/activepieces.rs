...
//!
//! Handler name format: `ap://<piece>.<action>`, e.g. `ap://slack.send_channel_message`.
//!
...
mod tests {
    ...
    fn is_ap_handler_detects_prefix() {
        assert!(is_ap_handler("ap://slack.send_channel_message"));
    ...
    fn parse_handler_extracts_piece_and_action() {
        let (piece, action) = parse_handler("ap://slack.send_channel_message").unwrap();
        assert_eq!(piece, "slack");
        assert_eq!(action, "send_channel_message");
    }
    ...
    fn parse_handler_rejects_missing_prefix() {
        let err = parse_handler("slack.send_channel_message").unwrap_err();
        assert!(matches!(err, StepError::Permanent { .. }));
    }
