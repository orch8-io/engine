...
use orch8_types::sequence::HumanInputDef;
...
pub struct StepContext {
    ...
    /// Cloned from `StepDef::wait_for_input` at dispatch time. Consumed by
...
pub struct HandlerRegistry {
    ...
    /// success/failure after the call. Absent in tests that don't exercise
    /// reliability behaviour so the breaker doesn't interfere with other
    /// assertions.
...
impl HandlerRegistry {
    ...
    /// Builder-style setter that installs a circuit breaker registry. The
    /// step dispatch sites call [`Self::circuit_breakers`] to obtain it; if
    /// unset, breaker checks are silently skipped (closed-by-default).
    #[must_use]
